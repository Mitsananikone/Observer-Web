...
export default function DropdownPlasticSurgery() {
  const [showBreastDropdown, setShowBreastDropdown] = useState(false);
  const [showBodyDropdown, setShowBodyDropdown] = useState(false);
  const [showFaceDropdown, setShowFaceDropdown] = useState(false);

  const renderPlasticSurgeryDropdown = () => {
    return (
      <ul className="bg-white border rounded-sm transform scale-0 absolute w-40 transition duration-500 ease-in-out origin-top group-hover:scale-100">
        <li 
          className="rounded-sm relative px-3 py-1 hover:bg-gray-100" 
          onMouseEnter={() => setShowBreastDropdown(true)}
          onMouseLeave={() => setShowBreastDropdown(false)}
        >
          ...
          {showBreastDropdown && (
            <div className="absolute left-full top-0 mt-2 w-56 rounded-md shadow-lg">
